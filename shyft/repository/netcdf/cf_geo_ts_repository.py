"""
Module for reading CF-compliant netcdf dataset needed for a
SHyFT run.
"""
...
class GeoTsRepository(interfaces.GeoTsRepository):
    ...
    def __init__(self, params):
        metstation_filepath = params["stations_met"]
        if not os.path.isabs(metstation_filepath):
            # Relative paths will be prepended the data_dir
            metstation_filepath = os.path.join(params['data_dir'], metstation_filepath)
        self.initialize(params,metstation_filepath,"")

    def initialize(self, params, metstation_filepath, discharge_filepath, raise_if_no_date=False):
        """
        Parameters
        ----------
        params: dict
            key "types" contain(s a dict list with
              "type": string, any of
